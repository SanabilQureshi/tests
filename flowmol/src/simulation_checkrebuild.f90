...
subroutine simulation_checkrebuild(rebuild)
    ...
    case(0) !Rapaport vmax criterion
	    ...
	    !$OMP PARALLEL DO IF(np .ge. 4096) DEFAULT(SHARED) PRIVATE(n) SCHEDULE(STATIC)
    ...
    case(2) !displacement
        ...
        block
            real(kind(0.d0)) :: rmax_thr
            integer :: rebuild_loc
            rmax_thr = 0.5d0*minval(delta_rneighbr)
            rebuild_loc = 0
            !$OMP PARALLEL DO IF(np .ge. 4096) DEFAULT(SHARED) PRIVATE(n) &
            !$OMP             REDUCTION(max:rebuild_loc) SCHEDULE(STATIC)
            do n = 1, np
                rdisp(:,n) = rdisp(:,n) + v(:,n) * delta_t
                if (any(abs(rdisp(:,n)) .gt. rmax_thr)) then
                    rebuild_loc = 1
                endif
            enddo
            !$OMP END PARALLEL DO
            if (rebuild_loc .eq. 1) rebuild = 1
        end block
    end select
