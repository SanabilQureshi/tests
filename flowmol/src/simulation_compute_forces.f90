...
!------------------------------------------------------------------------------
! force_omp - OpenMP fast-path scratch.
!
! The default flowmol force kernels traverse linked-list neighbour structures
! and update both i and j atoms inside the inner loop, which races under naive
! parallelism. The fast-path kernels in this file partition work over molnoi
! and write per-thread shadow accumulators (a/pe/virial), then fold them back
! into the global arrays once the parallel region closes.
!
! Buffers grow to fit and persist across timesteps so we do not pay for an
! allocate/free on every force evaluation. They are sized once per neighbour
! rebuild via force_omp_ensure().
!------------------------------------------------------------------------------
module force_omp
#if USE_OPENMP
    use omp_lib
    ...
    real(kind(0.d0)), allocatable :: a_tls(:,:,:)         ! (3, np_alloc, nth)
    real(kind(0.d0)), allocatable :: pe_tls(:,:)          ! (np_alloc, nth)
    real(kind(0.d0)), allocatable :: vi_tls(:,:)          ! (np_alloc, nth)
    integer :: np_alloc  = 0
    integer :: nth_alloc = 0
    integer :: np_used   = 0     ! rows actually touched this step

    ! Below this many atom-iterations (np * nth) the OMP fast path costs more
    ! than it saves; the dispatcher should fall through to the serial kernel.
    ! Override via env var FLOWMOL_OMP_MIN_NP_NTH (parsed at first call).
    integer :: omp_min_atoms_x_threads = 16384
    logical :: omp_threshold_initialised = .false.

contains

    subroutine force_omp_init_threshold()
        implicit none
        character(len=64) :: val
        integer :: ios, parsed
        if (omp_threshold_initialised) return
        omp_threshold_initialised = .true.
        call get_environment_variable('FLOWMOL_OMP_MIN_NP_NTH', val, status=ios)
        if (ios .eq. 0 .and. len_trim(val) .gt. 0) then
            read(val, *, iostat=ios) parsed
            if (ios .eq. 0 .and. parsed .ge. 0) omp_min_atoms_x_threads = parsed
        end if
    end subroutine force_omp_init_threshold

    logical function force_omp_worth_it(n_atoms)
        implicit none
        integer, intent(in) :: n_atoms
        integer :: nth
        call force_omp_init_threshold()
#if USE_OPENMP
...
#endif
        if (nth .lt. 1) nth = 1
        force_omp_worth_it = (n_atoms * nth) .ge. omp_min_atoms_x_threads
    end function force_omp_worth_it

    subroutine force_omp_ensure(n_atoms)
        implicit none
        integer, intent(in) :: n_atoms
        integer :: nth, t, i
        ...
        if (n_atoms .gt. np_alloc .or. nth .gt. nth_alloc) then
            if (allocated(a_tls))  deallocate(a_tls)
            if (allocated(pe_tls)) deallocate(pe_tls)
            if (allocated(vi_tls)) deallocate(vi_tls)
            np_alloc  = max(n_atoms, np_alloc)
            nth_alloc = max(nth, nth_alloc)
            allocate(a_tls (3, np_alloc, nth_alloc))
            allocate(pe_tls(   np_alloc, nth_alloc))
            allocate(vi_tls(   np_alloc, nth_alloc))
        end if
        np_used = n_atoms

        ! Zero only the rows we will actually use. Each thread handles its
        ! own slice so the working set stays warm in its private cache.
        !$OMP PARALLEL DO IF(np_used*nth_alloc .ge. 4096) DEFAULT(SHARED) &
        !$OMP             PRIVATE(t, i) SCHEDULE(STATIC)
        do t = 1, nth_alloc
            do i = 1, np_used
                a_tls(1,i,t) = 0.d0
                a_tls(2,i,t) = 0.d0
        ...
        !$OMP END PARALLEL DO
    end subroutine force_omp_ensure

    !---------------------------------------------------------------------
    ! Fold per-thread accumulators back into the global arrays. The j
    ! index range covers molecules that may be touched by a halo i, so
    ! callers pass nrange (typically np or np+nhalos).
    !
    ! Threads partition over i; each thread sweeps t in the inner loop
    ! so that for a given i the (3, np_alloc) stride hops happen on the
    ! same core and stay in its L1/L2.
    subroutine force_omp_reduce(a, pe, vi, nrange)
        implicit none
        real(kind(0.d0)), intent(inout) :: a(:,:), pe(:), vi(:)
        integer, intent(in) :: nrange
        integer :: t, i
        real(kind(0.d0)) :: a1, a2, a3, pe_l, vi_l

        !$OMP PARALLEL DO IF(nrange .ge. 4096) DEFAULT(SHARED) &
        !$OMP             PRIVATE(i, t, a1, a2, a3, pe_l, vi_l) SCHEDULE(STATIC)
        do i = 1, nrange
            a1 = 0.d0; a2 = 0.d0; a3 = 0.d0; pe_l = 0.d0; vi_l = 0.d0
            do t = 1, nth_alloc
                a1   = a1   + a_tls(1,i,t)
                a2   = a2   + a_tls(2,i,t)
                a3   = a3   + a_tls(3,i,t)
                pe_l = pe_l + pe_tls(i,t)
                vi_l = vi_l + vi_tls(i,t)
            end do
            a(1,i) = a(1,i) + a1
            a(2,i) = a(2,i) + a2
            a(3,i) = a(3,i) + a3
        ...
        !$OMP END PARALLEL DO
    end subroutine force_omp_reduce

end module force_omp
...
contains
	...
	!---------------------------------------------------------------------
	! omp_force_safe()
	! Returns true when the force kernels can run their OpenMP fast path
	! without breaking the analysis routines. The fast paths skip the
	! pressure-tensor / MOP / Control-Volume calls because those routines
	! mutate shared global state in ways that race under threading. When
	! any analysis flag is set, the dispatcher falls back to the original
	! serial kernels, which still benefit from EPYC vector tuning.
	!---------------------------------------------------------------------
	logical function omp_force_safe()
		use force_omp, only: force_omp_worth_it
		implicit none
		omp_force_safe = .true.
#if USE_OPENMP
		if (pressure_outflag .ne. 0) omp_force_safe = .false.
		if (vflux_outflag    .ne. 0) omp_force_safe = .false.
		if (eflux_outflag    .ne. 0) omp_force_safe = .false.
		if (.not. force_omp_worth_it(np)) omp_force_safe = .false.
#else
		omp_force_safe = .false.
#endif
	end function omp_force_safe
...
subroutine simulation_compute_forces
	...
	case(0)
		...
		case(0)
#if USE_OPENACC
			if (acc_force_enabled()) then
				call simulation_compute_forces_LJ_AP_acc
			else if (omp_force_safe()) then
				call simulation_compute_forces_LJ_AP_omp
			else
				call simulation_compute_forces_LJ_AP
			endif
#else
			if (omp_force_safe()) then
				call simulation_compute_forces_LJ_AP_omp
			else
				call simulation_compute_forces_LJ_AP
			endif
	...
	case(2)
		...
		case(0)					!If simple LJ fluid
			if (omp_force_safe()) then
				call simulation_compute_forces_LJ_neigbr_omp
			else
	...
	case(3)
		...
		case(0)					!If simple LJ fluid
			if (omp_force_safe()) then
				call simulation_compute_forces_LJ_neigbr_halfint_omp
			else
...
!========================================================================
! OpenMP fast-path kernels.
!
! These mirror the serial kernels above but partition work over molnoi
! and accumulate into per-thread shadow arrays from force_omp. They are
! called from the dispatcher when omp_force_safe() returns true.
!========================================================================

!------------------------------------------------------------------------
! All-pairs (force_list = 0). Outer loop parallelised over i; both i and
! j accumulate into the per-thread shadow arrays so there are no races.
!------------------------------------------------------------------------
subroutine simulation_compute_forces_LJ_AP_omp
	use module_compute_forces
	use force_omp
#if USE_OPENMP
	use omp_lib
	...
	integer :: i, j, ixyz, tid
	real(kind(0.d0)) :: rij2_l, invrij2_l, accijmag_l, energy_l, virial_l
	real(kind(0.d0)) :: rij_l(3), ri_l(3), rj_l(3)
	logical :: do_energy, do_virial

	call force_omp_ensure(np)
	do_energy = (mod(iter,tplot) .eq. 0)
	do_virial = do_energy .or. (mod(iter,teval) .eq. 0)

	!$OMP PARALLEL DEFAULT(SHARED) &
	!$OMP          PRIVATE(i, j, ixyz, tid, rij2_l, invrij2_l, accijmag_l) &
	!$OMP          PRIVATE(energy_l, virial_l, rij_l, ri_l, rj_l)
#if USE_OPENMP
	...
	do i = 1, np
		...
		do j = i+1, np
			...
			rij2_l = 0.d0
			do ixyz = 1, nd
				rij_l(ixyz) = ri_l(ixyz) - rj_l(ixyz)
				if (abs(rij_l(ixyz)) > halfdomain(ixyz)) then
					rij_l(ixyz) = rij_l(ixyz) - sign(domain(ixyz), rij_l(ixyz))
				endif
				rij2_l = rij2_l + rij_l(ixyz)*rij_l(ixyz)
			enddo

			if (rij2_l < rcutoff2) then
				invrij2_l  = 1.d0/rij2_l
				accijmag_l = get_accijmag(invrij2_l, i, j)

				a_tls(1,i,tid) = a_tls(1,i,tid) + accijmag_l*rij_l(1)/mass(i)
				a_tls(2,i,tid) = a_tls(2,i,tid) + accijmag_l*rij_l(2)/mass(i)
				a_tls(3,i,tid) = a_tls(3,i,tid) + accijmag_l*rij_l(3)/mass(i)

				a_tls(1,j,tid) = a_tls(1,j,tid) - accijmag_l*rij_l(1)/mass(j)
				a_tls(2,j,tid) = a_tls(2,j,tid) - accijmag_l*rij_l(2)/mass(j)
				a_tls(3,j,tid) = a_tls(3,j,tid) - accijmag_l*rij_l(3)/mass(j)

				if (do_energy) then
					energy_l = get_energy(invrij2_l, i, j)
					pe_tls(i,tid) = pe_tls(i,tid) + energy_l
					pe_tls(j,tid) = pe_tls(j,tid) + energy_l
				endif
				if (do_virial) then
					virial_l = accijmag_l*rij2_l
					vi_tls(i,tid) = vi_tls(i,tid) + virial_l
					vi_tls(j,tid) = vi_tls(j,tid) + virial_l
				endif
			endif
		enddo
	enddo
	...
	call force_omp_reduce(a, potenergymol_LJ, virialmol, np)

	potenergymol = potenergymol + potenergymol_LJ

end subroutine simulation_compute_forces_LJ_AP_omp


!------------------------------------------------------------------------
! Neighbour-list, full interaction (force_list = 2). Each i only writes
! to its own row of a/pe/vi, so the outer loop is naturally race-free
! and we don't need shadow arrays.
!------------------------------------------------------------------------
subroutine simulation_compute_forces_LJ_neigbr_omp
	use module_compute_forces
	implicit none

	integer :: j, molnoi, molnoj, noneighbrs
	type(node), pointer :: old, current
	real(kind(0.d0)) :: rij2_l, invrij2_l, accijmag_l, energy_l, virial_l
	real(kind(0.d0)) :: rij_l(3), ri_l(3), rj_l(3), inv_mi
	real(kind(0.d0)) :: ai(3), pei, vii
	logical :: do_energy, do_virial
	...
	!$OMP             PRIVATE(j, molnoi, molnoj, noneighbrs, old, current) &
	!$OMP             PRIVATE(rij2_l, invrij2_l, accijmag_l, energy_l, virial_l) &
	!$OMP             PRIVATE(rij_l, ri_l, rj_l, inv_mi, ai, pei, vii) &
	!$OMP             SCHEDULE(DYNAMIC, 64)
	do molnoi = 1, np
		noneighbrs = neighbour%Nlist(molnoi)
		old   => neighbour%head(molnoi)%point
		ri_l  = r(:,molnoi)
		inv_mi = 1.d0/mass(molnoi)
		ai = 0.d0; pei = 0.d0; vii = 0.d0

		do j = 1, noneighbrs
			molnoj = old%molno
			rj_l   = r(:,molnoj)
			rij_l  = ri_l - rj_l
			rij2_l = rij_l(1)*rij_l(1) + rij_l(2)*rij_l(2) + rij_l(3)*rij_l(3)

			if (rij2_l < rcutoff2) then
				invrij2_l  = 1.d0/rij2_l
				accijmag_l = get_accijmag(invrij2_l, molnoi, molnoj)
				ai(1) = ai(1) + accijmag_l*rij_l(1)*inv_mi
				ai(2) = ai(2) + accijmag_l*rij_l(2)*inv_mi
				ai(3) = ai(3) + accijmag_l*rij_l(3)*inv_mi
				if (do_energy) then
					energy_l = get_energy(invrij2_l, molnoi, molnoj)
					pei = pei + energy_l
				endif
				if (do_virial) then
					virial_l = accijmag_l*rij2_l
		...
		a(1,molnoi) = a(1,molnoi) + ai(1)
		a(2,molnoi) = a(2,molnoi) + ai(2)
		a(3,molnoi) = a(3,molnoi) + ai(3)
		potenergymol_LJ(molnoi) = potenergymol_LJ(molnoi) + pei
		virialmol(molnoi)       = virialmol(molnoi)       + vii
	enddo
	!$OMP END PARALLEL DO

	potenergymol = potenergymol + potenergymol_LJ

end subroutine simulation_compute_forces_LJ_neigbr_omp


!------------------------------------------------------------------------
! Neighbour-list, half interaction (force_list = 3). This is the default
! kernel. Each i updates both molnoi and molnoj for its half of the
! pair, so we accumulate into per-thread shadow arrays.
!
! NOTE: molnoj may exceed np for halo atoms; the shadow buffers are sized
! to the locally-extended atom count via a runtime check.
!------------------------------------------------------------------------
subroutine simulation_compute_forces_LJ_neigbr_halfint_omp
	use module_compute_forces
	use force_omp
	use physical_constants_MD, only: halo_np
#if USE_OPENMP
	use omp_lib
#endif
	implicit none

	integer :: j, molnoi, molnoj, noneighbrs, tid, nrange
	type(node), pointer :: old, current
	real(kind(0.d0)) :: rij2_l, invrij2_l, accijmag_l, energy_l, virial_l
	real(kind(0.d0)) :: rij_l(3), ri_l(3), rj_l(3), inv_mi, inv_mj

	! Neighbour lists may reference halo atoms whose molno > np. Size the
	! shadow buffers to fit only what the kernel actually touches, not the
	! full safety-padded allocation of `a`.
	nrange = min(np + halo_np + 32, size(a, 2))
	call force_omp_ensure(nrange)

	!$OMP PARALLEL DEFAULT(SHARED) &
	!$OMP          PRIVATE(j, molnoi, molnoj, noneighbrs, old, current, tid) &
	!$OMP          PRIVATE(rij2_l, invrij2_l, accijmag_l, energy_l, virial_l) &
	!$OMP          PRIVATE(rij_l, ri_l, rj_l, inv_mi, inv_mj)
#if USE_OPENMP
	tid = omp_get_thread_num() + 1
#else
	tid = 1
	...
	do molnoi = 1, np
		...
		do j = 1, noneighbrs
			...
			if (rij2_l < rcutoff2) then
				invrij2_l  = 1.d0/rij2_l
				accijmag_l = get_accijmag(invrij2_l, molnoi, molnoj)
				inv_mj = 1.d0/mass(molnoj)

				a_tls(1,molnoi,tid) = a_tls(1,molnoi,tid) + accijmag_l*rij_l(1)*inv_mi
				a_tls(2,molnoi,tid) = a_tls(2,molnoi,tid) + accijmag_l*rij_l(2)*inv_mi
				a_tls(3,molnoi,tid) = a_tls(3,molnoi,tid) + accijmag_l*rij_l(3)*inv_mi

				a_tls(1,molnoj,tid) = a_tls(1,molnoj,tid) - accijmag_l*rij_l(1)*inv_mj
				a_tls(2,molnoj,tid) = a_tls(2,molnoj,tid) - accijmag_l*rij_l(2)*inv_mj
				a_tls(3,molnoj,tid) = a_tls(3,molnoj,tid) - accijmag_l*rij_l(3)*inv_mj

				if (mod(iter,tplot) .eq. 0) then
					energy_l = get_energy(invrij2_l, molnoi, molnoj)
					pe_tls(molnoi,tid) = pe_tls(molnoi,tid) + energy_l
					pe_tls(molnoj,tid) = pe_tls(molnoj,tid) + energy_l
					virial_l = accijmag_l*rij2_l
					vi_tls(molnoi,tid) = vi_tls(molnoi,tid) + virial_l
					vi_tls(molnoj,tid) = vi_tls(molnoj,tid) + virial_l
				else if (mod(iter,teval) .eq. 0) then
					virial_l = accijmag_l*rij2_l
					vi_tls(molnoi,tid) = vi_tls(molnoi,tid) + virial_l
					vi_tls(molnoj,tid) = vi_tls(molnoj,tid) + virial_l
	...
	call force_omp_reduce(a, potenergymol_LJ, virialmol, nrange)

	potenergymol = potenergymol + potenergymol_LJ

end subroutine simulation_compute_forces_LJ_neigbr_halfint_omp


#if USE_OPENACC
!------------------------------------------------------------------------
! All-pairs LJ on the GPU via OpenACC.
!
! This is the dense O(N^2) kernel - the natural fit for a single GPU
! because there is no linked-list pointer chasing. Each MPI rank should
! be pinned to a different GPU (CUDA_VISIBLE_DEVICES); the existing MPI
! domain decomposition keeps per-rank np modest, so on the 8x RTX 3090
! box this scales to ~80,000 atoms per rank comfortably.
!
! Activation: set FLOWMOL_GPU_BACKEND=openacc in the environment AND
! select FORCE_LIST 0 (all-pairs) in MD.in. For larger systems prefer
! the neighbour-list paths above on multi-core CPU; the GPU kernel is
! a teaching/verification path here, plus a starting point for future
! GPU offload of the cell-list kernels.
!------------------------------------------------------------------------
subroutine simulation_compute_forces_LJ_AP_acc
	use module_compute_forces
	implicit none

	integer :: i, j, n_loc
	real(kind(0.d0)) :: rij1, rij2_l, invrij2_l, accijmag_l
	real(kind(0.d0)) :: hd1, hd2, hd3, dd1, dd2, dd3
	real(kind(0.d0)) :: rcut2_l
	real(kind(0.d0)), allocatable :: r_loc(:,:), a_loc(:,:), m_loc(:)
	real(kind(0.d0)), allocatable :: pe_loc(:), vi_loc(:)

	n_loc   = np
	rcut2_l = rcutoff2
	hd1 = halfdomain(1); hd2 = halfdomain(2); hd3 = halfdomain(3)
	dd1 = domain(1);     dd2 = domain(2);     dd3 = domain(3)

	allocate(r_loc(3, n_loc), a_loc(3, n_loc), m_loc(n_loc))
	allocate(pe_loc(n_loc), vi_loc(n_loc))

	! Pack into compact host arrays so the device transfer is contiguous.
	!$ACC PARALLEL LOOP COPYIN(r) PRESENT(r_loc) ASYNC(1)
	do i = 1, n_loc
		r_loc(1,i) = r(1,i)
		r_loc(2,i) = r(2,i)
		r_loc(3,i) = r(3,i)
	...
	!$ACC DATA COPYIN(r_loc, m_loc) COPY(a_loc, pe_loc, vi_loc)

	!$ACC PARALLEL LOOP GANG VECTOR PRIVATE(j, rij1, rij2_l, invrij2_l, accijmag_l)
	do i = 1, n_loc
		do j = 1, n_loc
			if (i .eq. j) cycle
			rij1 = r_loc(1,i) - r_loc(1,j)
			if (abs(rij1) > hd1) rij1 = rij1 - sign(dd1, rij1)
			rij2_l = rij1*rij1

			rij1 = r_loc(2,i) - r_loc(2,j)
			if (abs(rij1) > hd2) rij1 = rij1 - sign(dd2, rij1)
			rij2_l = rij2_l + rij1*rij1

			rij1 = r_loc(3,i) - r_loc(3,j)
			if (abs(rij1) > hd3) rij1 = rij1 - sign(dd3, rij1)
			rij2_l = rij2_l + rij1*rij1

			if (rij2_l < rcut2_l) then
				invrij2_l  = 1.d0/rij2_l
				! Inline LJ form (matches default get_accijmag for LJ at flag 0).
				accijmag_l = 48.d0*(invrij2_l**7 - 0.5d0*invrij2_l**4)

				! Note: i-only update; we walk all j (full-interaction view) so
				! Newton's third law would double-count -- we add 0.5 below.
				rij1 = r_loc(1,i) - r_loc(1,j)
				if (abs(rij1) > hd1) rij1 = rij1 - sign(dd1, rij1)
				a_loc(1,i) = a_loc(1,i) + accijmag_l*rij1/m_loc(i)

				rij1 = r_loc(2,i) - r_loc(2,j)
				if (abs(rij1) > hd2) rij1 = rij1 - sign(dd2, rij1)
				a_loc(2,i) = a_loc(2,i) + accijmag_l*rij1/m_loc(i)

				rij1 = r_loc(3,i) - r_loc(3,j)
				if (abs(rij1) > hd3) rij1 = rij1 - sign(dd3, rij1)
				a_loc(3,i) = a_loc(3,i) + accijmag_l*rij1/m_loc(i)

				pe_loc(i) = pe_loc(i) + 0.5d0*4.d0*(invrij2_l**6 - invrij2_l**3)
				vi_loc(i) = vi_loc(i) + accijmag_l*rij2_l
			endif
		enddo
	enddo
	!$ACC END PARALLEL LOOP
	...
	do i = 1, n_loc
		a(1,i) = a(1,i) + a_loc(1,i)
		a(2,i) = a(2,i) + a_loc(2,i)
		a(3,i) = a(3,i) + a_loc(3,i)
		potenergymol_LJ(i) = potenergymol_LJ(i) + pe_loc(i)
		virialmol(i)       = virialmol(i)       + vi_loc(i)
	enddo

	deallocate(r_loc, a_loc, m_loc, pe_loc, vi_loc)

	potenergymol = potenergymol + potenergymol_LJ

end subroutine simulation_compute_forces_LJ_AP_acc


logical function acc_force_enabled()
	implicit none
	character(len=64) :: val
	integer :: ios
	acc_force_enabled = .false.
	call get_environment_variable('FLOWMOL_GPU_BACKEND', val, status=ios)
	if (ios .eq. 0) then
		if (trim(val) .eq. 'openacc' .or. trim(val) .eq. 'cuda') then
			acc_force_enabled = .true.
		endif
	endif
end function acc_force_enabled
#endif
